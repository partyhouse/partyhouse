...
VENV = os.path.join(ROOT, '.partyhouse-venv')
...
def install_dependencies(venv=VENV):
  ...
  # Tell the virtual env how to "import partyhouse"
  #py = 'python%d.%d' % (sys.version_info[0], sys.version_info[1])
  #pthfile = os.path.join(venv, "lib", py, "site-packages", "partyhouse.pth")
  #f = open(pthfile, 'w')
  #f.write("%s\n" % ROOT)
...
def print_summary():
  summary = """
 Partyhouse development environment setup is complete.

 To activate the virtualenv for the extent of your current shell session you
 can run:

 $ source .partyhouse-venv/bin/activate
  """
  print summary
